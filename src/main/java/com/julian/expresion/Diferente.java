...
/*
    * Clase que representa una expresión de comparación de diferente.
    * Valor = 1 si es verdadero, 0 si es falso.
 * */
public class Diferente extends Instruccion {

    private Instruccion operIzq;
    private Instruccion operDer;

    /**
     * Constructor de la clase Diferente.
     ...
     */
    public Diferente(Instruccion operIzq, Instruccion operDer, int linea, int columna) {
        ...
        switch (tipoIzq) {
            ...
            case CADENA -> {
                switch (tipoDer) {
                    case CADENA -> {
                        this.tipo.setTipo(tipoDato.BOOLEANO);
                        return !valorIzq.equals(valorDer) ? 1 : 0;
                    }
                    default -> {
                        return new Errores("Semantico", "Error en la diferencia, tipo de dato no valido", this.linea, this.columna);
                    }
